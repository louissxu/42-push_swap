  ...
  while l:
    for _ in range(len(l)):
      if l[0] <= low_cut or l[0] >= high_cut:
        ...
        ra(l, r, moves)
    # print(f"{low_cut} to {high_cut}    {high_cut + (group_size * groups_per_pre_group // 2)} {low_cut - (group_size * groups_per_pre_group // 2)}")
    # FIX: DO THIS MATH PROPERLY. CURRENTY CRUDE FIX TO INCLUDE THE VALUES TO AVOID SMALL CENTRE NOT VISITED
...
if len(l) == 0:
  ...
  # m.extend(selection_sort_back_2(l, r))
  m.extend(selection_sort_back_3(l, r, 20))
else:
  m.extend(double_radix_sort_improved(l, r))
